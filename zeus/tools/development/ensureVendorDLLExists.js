...
function getJsFilesFromSrcDir(rootPath, srcPath) {
  return globSync(`${pathResolve(rootPath, srcPath)}/**/*.js`);
}

function buildVendorDLL() {
  return new Promise((resolve, reject) => {
    Promise.all([
      Promise.resolve(getJsFilesFromSrcDir(moduleRootPath, 'src/client')),
      Promise.resolve(getJsFilesFromSrcDir(moduleRootPath, 'src/shared/universal')),
      Promise.resolve(getJsFilesFromSrcDir(appRootPath, 'app')),
    ])
    .then(([clientFiles, universalFiles, appFiles]) => {
      const isJsFile = file => pathExtName(file) === '.js';
      const allJSFiles = [...clientFiles, ...universalFiles, ...appFiles].filter(isJsFile);
